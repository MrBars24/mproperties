...
if(isset($_POST['submit'])){
	...
	if(filter_var($email, FILTER_VALIDATE_EMAIL)){
		...
		$chkemail = $conn->prepare("SELECT * FROM user_emails WHERE emails = :email");

		$chkemail->execute([":email" => $email]);

		$result = $chkemail->fetchAll();

		

		if(count($result) > 0){

			$error = "You've already Sign up";

			$alert = "danger";

		}else{

			$insert = $conn->prepare("INSERT INTO user_emails(emails) VALUES(:email)");

			if($insert->execute([":email" => $email])){

				$error = "Thank you for RSVP to our soft launch event. <br/> We are looking forward to a great time!";
...
.alert {
	...
	<script src="js/vendor/jquery.final-countdown.js"></script>
	...
	<script>
		...
		$(function() {
		  ...
		  var current = new Date().getTime();
            ...
            future = new Date("Nov 7, 2018 12:55").getTime();
	    	...
	    	$('.countdown').final_countdown({
                ...
                $.ajax({
					url : 'http://localhost:8081/microproperties/page/action/set_live',
					success : function(res) {
						window.location.href = "http://localhost:8081/microproperties";
					}
				})
        	});
