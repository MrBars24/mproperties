...
class PropertyManagers extends CI_Controller {
	...
	public function index(){
		...
		$this->load->view('admin/property-managers-tpl',$data);
	}   
    ...
    function form($user_id = false)
    {
        ...
        {
            $this->load->view('admin/property-managers-form-tpl',$data);
            ...
            if ($user_id != 0) {
                ...
                $this->session->set_flashdata("property_manager_updated", "Property Manager Updated");
                ...
                $this->ion_auth->update($user_id, $save_data);
                $this->session->set_flashdata("property_manager_added", "Property Manager Added");

            }

            redirect('admin/property-managers');
        }
    ...
    public function ajax($section){
        ...
        switch ($section) {
            ...
            case 'pmanager_delete':
                $this->pmanager_delete();
                break;
        }
    }
    ...
    protected function get_all_users()
    {
        ...
        $users = $this->Users_model->get_all_users($first_name, $last_name, $username, $email, false, false, $order_by, 3);
        ...
        for($i = $iDisplayStart; $i < $end; $i++) {
            ...
            $records["data"][] = array(
                ...
                '<a href="'.site_url("admin/property-managers/form/".$user[$i]->user_id).'" class="btn btn-sm btn-circle btn-default btn-editable"><i class="fa fa-pencil"></i> Edit</a><a href="javascript:void(0);" class="btn btn-sm btn-circle btn-default btn-editable" onclick="deletePropertyManager('.$user[$i]->user_id.')">Delete</a>',
            );
        }
    ...
    protected function pmanager_delete()
    {
        $user_id = $this->input->post('id');

        $this->Users_model->delete_user($user_id);
        $this->session->set_flashdata("property_manager_deleted", "Property Manager Deleted");
