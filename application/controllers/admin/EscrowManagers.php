...
class EscrowManagers extends CI_Controller {
	...
	public function index() {
		...
		$this->load->view('admin/escrow-manager-tpl', $data);
    }
    ...
    */
    function form($user_id = false)
    {
        ...
        {
            $this->load->view('admin/escrow-managers-form-tpl',$data);
            ...
            if ($user_id != 0) {
                ...
                $this->session->set_flashdata("escorw_manager_updated", "Escrow Manager Updated");
            }else{
                ...
                $this->session->set_flashdata("escorw_manager_added", "Escrow Manager Added");
                $this->ion_auth->update($user_id, $save_data);

            }

            redirect('admin/escrow-managers');
        }
    ...
    /**
     * GET
     * route : admin/escrow-managers/ajax/(.*)
     *
     * Ajax functionality for Escrow Managers
     * 
     * @param mixed $section
     * @return void
     */
    public function ajax($section){
        ...
        switch ($section) {
            ...
            case 'emanager_delete':
                $this->emanager_delete();
                break;
        }
    }
    ...
    protected function get_all_users()
    {
        ...
        $users = $this->Users_model->get_all_users($first_name, $last_name, $username, $email, false, false, $order_by, 9);
        ...
        for($i = $iDisplayStart; $i < $end; $i++) {
            ...
            $records["data"][] = array(
                ...
                '<a href="'.site_url("admin/escrow-managers/form/".$user[$i]->user_id).'" class="btn btn-sm btn-circle btn-default btn-editable"><i class="fa fa-pencil"></i> Edit</a><a href="javascript:void(0);" class="btn btn-sm btn-circle btn-default btn-editable" onclick="deleteEscrowManager('.$user[$i]->user_id.')">Delete</a>',
            );
        }
    ...
    protected function emanager_delete()
    {
        $user_id = $this->input->post('id');

        $this->Users_model->delete_user($user_id);
        $this->session->set_flashdata("escorw_manager_deleted", "Escrow Manager Deleted");
