...
class RealEstateAgents extends CI_Controller {
	...
	public function index(){
		...
		$this->load->view('admin/real-estate-agents-tpl',$data);
	}   
    ...
    function form($user_id = false)
    {
        ...
        {
            $this->load->view('admin/real-estate-agents-form-tpl',$data);
            ...
            if ($user_id != 0) {
                ...
                $this->session->set_flashdata("real_state_updated", "Real State Agent Updated");
                ...
                $this->ion_auth->update($user_id, $save_data);
                $this->session->set_flashdata("real_state_added", "Real State Agent Added");


                //$this->Users_model->insert_real_estate_agent($user_id);

            }

            redirect('admin/real-estate-agents');
        }
    ...
    public function ajax($section){
        switch ($section) {
            case 'get_all_users':
                ...
                $users = $this->Users_model->get_all_users($first_name, $last_name, $username, $email, false, false, $order_by, 6);
                ...
                for($i = $iDisplayStart; $i < $end; $i++) {
                    ...
                    $records["data"][] = array(
                        ...
                        '<a href="'.site_url("admin/real-estate-agents/form/".$user[$i]->user_id).'" class="btn btn-sm btn-circle btn-default btn-editable"><i class="fa fa-pencil"></i> Edit</a><a href="javascript:void(0);" class="btn btn-sm btn-circle btn-default btn-editable" onclick="deletePropertyManager('.$user[$i]->user_id.')">Delete</a>',
                    );
                }
            ...
            case 'agent_delete':
                ...
                $this->session->set_flashdata("real_state_deleted", "Real State Agent Deleted");
